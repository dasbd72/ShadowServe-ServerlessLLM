...
class RoundRobinRouter(SllmRouter):
    ...
    async def inference(self, request_data: dict, action: str):
        ...
        logger.info(
            f"Enqueued {action} request for model {self.model_name} "
            f"ts={_event_ts()}"
        )
    ...
    async def _create_instance(self):
        ...
        logger.info(
            ...
            f"max queue length is {max_request_length} ts={_event_ts()}"
        )
    ...
    async def _start_instance(self, instance_id):
        ...
        try:
            ...
            elif self.shadow_scale_up and self.backend == "vllm":
                results = await self._start_all_backend_instances(
    ...
    async def _start_all_backend_instances(
    ...
    async def _on_shadow_scale_up(
        ...
        logger.info(
            "Shadow scale-up migration_id=%s source=%s instance=%s ts=%s",
            migration_id,
            source.instance_id,
        ...
        start_backend_instance_task = asyncio.create_task(
            self._start_all_backend_instances(startup_node, instance)
        )
        ...
        logger.info(
            "Shadow scale-up KVHTS migrate started migration_id=%s "
            "requests=%s ts=%s",
            migration_id,
            migrated_requests,
            _event_ts(),
        ...
        logger.info(
            "Shadow scale-up KVHTS migrate completed migration_id=%s ts=%s",
            migration_id,
            _event_ts(),
        )
        ...
        logger.info(
            "Shadow scale-up KVSTC migrate completed migration_id=%s ts=%s",
            migration_id,
            _event_ts(),
        )
