...
At most ``router_config.target`` requests are migrated per scale-up event.

Prerequisites:
  1. Ray + SLLM running
    (e.g. ``python examples/start_sllm.py --deploy --config <config-file>.json``).
...
Example:
  ...
  python examples/shadow_scale_up/simple_client.py --concurrent 2 --max-tokens 2048
"""
...
class Context:
    ...
    async def run(self) -> None:
        ...
        for i in range(self.concurrent):
            if self.stagger_ms > 0 and i > 0:
                futures.append(
                    asyncio.create_task(
                        self.send_chat_delay(i, self.stagger_ms / 1000.0)
                    )
                )
            else:
    ...
    async def send_chat_delay(self, request_idx: int, delay: float) -> dict:
        await asyncio.sleep(delay)
        return await self.send_chat(request_idx)
